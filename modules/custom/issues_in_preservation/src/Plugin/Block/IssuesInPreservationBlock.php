...
class IssuesInPreservationBlock extends BlockBase {
  ...
  public function build () {
     \Drupal::logger ('issues')->notice ('[build] issues: <pre>' . '</pre>');
  ...
  private function getIssuesInPreservation () {
    ...
    foreach (array_keys ($result) as $nid) {
      ...
      $issues [] = array (
        ...
        'summary' => $node->get ('field_issue_summary')->value,
        'body' => $node->get ('body')->value
      );
    }

    \Drupal::logger ('issues_in_preservation')->notice ('[getIssuesInPreservation] issues: <pre>' . print_r ($issues, true) . '</pre>');
